...
#include "peakphysics/physics/Capsule.hpp"
...
namespace peak
{
	namespace physics
	{
		CharacterController::CharacterController() : simulation(0), body(0), shape(0),
			onground(false)
		{
		}
		...
		void CharacterController::init(Simulation *simulation)
		{
		...
		void CharacterController::update()
		{
			...
			bool onground = false;
			if (simulation->castRay(raystart, rayend, &collinfo, body))
			{
				onground = true;
				...
				if (speed.getLength() > 0)
				{
					...
					CharacterCallback cb(body->getBody(), from.getOrigin(),
						to.getOrigin());
					simulation->getWorld()->convexSweepTest(shape, from, to, cb);
					if (!cb.hasHit())
					{
						body->setPosition(body->getPosition() + Vector3F(speed.x / 20, 0.0f, speed.z / 20));
					}
				}
			}
			else
