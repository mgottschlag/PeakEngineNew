...
namespace peak
{
	namespace network
	{
		...
		bool NetworkHost::init(unsigned int port, unsigned int maxclients,
			unsigned int maxin, unsigned int maxout)
		{
			// Create network host
			ENetAddress address;
		...
		NetworkConnection* NetworkHost::getNewConnection()
		{
			...
			NetworkConnection *con = newconnections.front();
			newconnections.pop();
			return con;
		}
		...
		bool NetworkHost::update()
		{
			...
			while (enet_host_service(host, &event, 0) > 0)
			{
				...
				case ENET_EVENT_TYPE_RECEIVE:
				{
					...
					Buffer *buffer = new Buffer(event.packet->data,
